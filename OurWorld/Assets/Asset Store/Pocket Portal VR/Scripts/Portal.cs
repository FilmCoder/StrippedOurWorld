...
public class Portal : MonoBehaviour
{
    ...
    // made mainCamera public, for compatibility with Pocket Portal VR
...
/* Gear VR Special */
	...
	void Update ()
	{
        // if main camera wasn't found in Start(), it's possible it has now been created at runtime by VRTK
        if(!mainCamera) {
            if(Camera.main)
            {
                mainCamera = Camera.main;
                Debug.LogWarning("Main Camera found.");
            } else
            {
                Debug.LogWarning("Main Camera could not be found.");
            }
        }
